...
int main(int argc, char *argv[]) {
  ...
  cmdp.setOption("is_lap","is_weight",&is_lap, "is the input already the laplacian.");
  ...
  RCP<CrsMatrix> L;
  RCP<Vector> auxVec;
  formLaplacian(fileMat, useWeightedLaplacian, useNormalizedLaplacian, L, auxVec, is_lap);
...
void formLaplacian(const RCP<const CrsMatrix>& A, const bool weighted, const bool normalized, RCP<CrsMatrix>& L, RCP<Vector>& auxVec, const bool is_lap)
{
  ...
  L = Tpetra::MatrixMatrix::add(ONE,false,*A,ZERO,true,*A);
  RCP<const Tpetra::Map<Ordinal,Ordinal,Node> > rowMap = L->getRowMap();  
  if ( is_lap )
    {
      cout << "is already a laplacian!";
